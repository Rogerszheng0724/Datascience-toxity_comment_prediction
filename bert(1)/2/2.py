...
###############################################################################
# 主流程：對 toxicity 進行處理（訓練、預測、評估與視覺化）
###############################################################################
def process_toxicity(df, target_col, embedding_type='bert', w2v_model=None, tokenizer=None, bert_model=None):
    """
    對指定 target_col (toxicity) 進行處理：
      1. 前處理與向量化
      2. 資料切分：70% 訓練、15% 驗證、15% 測試
      3. Oversampling 改為 Undersampling（使用 RandomUnderSampler）
      4. 模型訓練、預測與評估（多類別：0～6）
      5. 產生混淆矩陣、Precision-Recall 與 ROC 曲線（micro-average）
    並顯示各階段所花費的時間。
    """
    print(f"\n===== Processing target: {target_col} with {embedding_type} =====")
    start_total = time.time()

    # 1. 前處理：停用詞 (對於 Word2Vec 使用 clean_tokens)
    df['clean_tokens'] = df['comment_text'].apply(remove_stopwords)
    
    # 2. 向量化：依據 embedding_type
    start_emb = time.time()
    ...
    # 5. Undersampling - 僅對訓練集
    rus = RandomUnderSampler(random_state=42)
    X_train_res, y_train_res = rus.fit_resample(X_train, y_train)
    
    # 6. 模型訓練：使用 multi_class='multinomial'
    ...
    plt.savefig(f"{target_col}_{embedding_type}_confusion_matrix.png")
    plt.close()
    ...
    plt.title(f"Micro-average Precision-Recall Curve ({embedding_type})")
    plt.legend(loc="lower left")
    plt.savefig(f"{target_col}_{embedding_type}_precision_recall.png")
    plt.close()
    ...
    plt.title(f"Micro-average ROC Curve ({embedding_type})")
    plt.legend(loc="lower right")
    plt.savefig(f"{target_col}_{embedding_type}_roc_curve.png")
    plt.close()
...
if __name__ == "__main__":
    # 1. 修改此處路徑為您的新資料集路徑（資料集需包含 comment_text 與 toxicity 欄位）
    ...
    # 5. 執行 toxicity 流程
    clf, report = process_toxicity(df.copy(), target_column, embedding_type=use_embedding,
                                   w2v_model=w2v_model,
                                   tokenizer=(tokenizer if use_embedding=='bert' else None),
                                   bert_model=(bert_model if use_embedding=='bert' else None))
