...
###############################################################################
# 主流程：利用 SMOTE + 停用詞刪除 + Word2Vec + XGBoost 預測多類別 toxicity (0～6)
###############################################################################
def process_toxicity(df, target_col, w2v_model):
    ...
    對指定 target_col (toxicity) 進行處理：
      ...
      3. 使用 SMOTE 對訓練集進行 oversampling
      4. 使用 XGBoost 訓練多類別模型（0～6）
      5. 預測與評估，並產生混淆矩陣、Precision-Recall 與 ROC 曲線（micro-average）
    並顯示各階段所花費的時間。
    """
    ...
    # 6. 模型訓練：使用 XGBoost (設定 objective 為 multi:softprob, num_class=7)
    start_train = time.time()
    clf = xgb.XGBClassifier(objective='multi:softprob', num_class=7, random_state=42, n_jobs=-1)
...
if __name__ == "__main__":
    ...
    file_path = r"C:\Users\Admin\Desktop\datascience\xgboost\train.csv"
    ...
    # 4. 執行 toxicity 流程（移除 embedding_type 參數，因為 process_toxicity() 不接受此參數）
