...
###############################################################################
# 主流程：利用 undersampling + 停用詞刪除 + Word2Vec + XGBoost 預測多類別 toxicity (0～6)
###############################################################################
def process_toxicity(df, target_col, w2v_model):
    """
    對指定 target_col (toxicity) 進行處理：
      1. 前處理與向量化（使用 Word2Vec）
      2. 資料切分：70% 訓練，剩下 30% 均分為驗證與測試（各 15%）
      3. 使用 RandomUnderSampler 進行 undersampling（訓練集）
      4. 利用 XGBoost 訓練多類別模型（objective='multi:softprob', num_class=7）
      5. 預測與評估：計算分類報告、ROC-AUC、混淆矩陣，以及 micro-average Precision-Recall 與 ROC 曲線
    並顯示各階段所花費的時間。
    """
    ...
    # 6. 模型訓練：使用 XGBoost (設定 objective='multi:softprob', num_class=7)
