...
###############################################################################
# 主流程：利用 oversampling + 停用詞刪除 + Word2Vec + 羅吉斯回歸 預測多類別 toxicity (0～6)
###############################################################################
def process_toxicity(df, target_col, w2v_model):
    ...
    對指定 target_col (toxicity) 進行處理：
      ...
      3. 使用 RandomOverSampler 進行 oversampling（訓練集）
    ...
    # 5. Oversampling - 僅對訓練集
    ros = RandomOverSampler(random_state=42)
    X_train_res, y_train_res = ros.fit_resample(X_train, y_train)
    
    # 6. 模型訓練：使用羅吉斯回歸（多類別設定）
...
if __name__ == "__main__":
    ...
    file_path = r"C:\Users\Admin\Desktop\datascience\logest\train.csv"
